...
void mandelbrot(void *devPtr, int w, int h, double cx, double cy, double zoom, int iter, bool double_prec)
{
    const int blockSize = 16; // 256 threads per block
    if(double_prec) {
        mandel_double<<<dim3(w / blockSize, h / blockSize), dim3(blockSize, blockSize)>>>((uchar4 *) devPtr, w, h, cx, cy, zoom, iter);
    }
    else {
        mandel_float<<<dim3(w / blockSize, h / blockSize), dim3(blockSize, blockSize)>>>((uchar4 *) devPtr, w, h, float(cx), float(cy), float(zoom), iter);
    }
}
